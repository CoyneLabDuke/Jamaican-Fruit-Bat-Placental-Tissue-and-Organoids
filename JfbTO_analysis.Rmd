...
```
```#Combine TOs with Tissue trophs
...
DefaultAssay(TOs) <- "RNA";   DefaultAssay(trophs) <- "RNA"
TOs    <- JoinLayers(TOs);    trophs <- JoinLayers(trophs)
TOs@assays    <- list(RNA = TOs[["RNA"]])
trophs@assays <- list(RNA = trophs[["RNA"]])

## Split per sample and run SCT WITHOUT regressing S/G2M (we’ll control at feature-selection)
objs_TOs    <- SplitObject(TOs,    split.by = "orig.ident")
objs_trophs <- SplitObject(trophs, split.by = "orig.ident")
objs_list   <- c(objs_TOs, objs_trophs)
...
objs_list <- lapply(
  ...
  \(x) SCTransform(
    ...
    verbose = TRUE,
    ncells = 50000,
    ## NOTE: SCTransform models depth; you usually do NOT need nCount_RNA.
    ## Keep your bat-specific QC covariates:
    vars.to.regress = c("percent.mt.custom","percent.rb"),
    return.only.var.genes = FALSE,
    method = "glmGamPoi"
  )
)

features_all  <- SelectIntegrationFeatures(object.list = objs_list, nfeatures = 3000)
features_nocc <- setdiff(features_all, unique(c(cc_s_genes, cc_g2m_genes)))

DefaultAssay(merged_sct) <- "SCT"
VariableFeatures(merged_sct) <- features_nocc
merged_sct <- RunPCA(merged_sct, features = features_nocc, npcs = 50)

merged_sct <- RunHarmony(
  merged_sct,
  group.by.vars      = "orig.ident",          
  covariates         = c("S.Score","G2M.Score"),
  dims.use           = 1:40,
  theta              = 4,                     
  nclust             = 75,
  max.iter.harmony   = 30,
...
merged_sct <- RunUMAP(merged_sct, reduction = "harmony", dims = 1:40, min.dist = 0.3)
merged_sct <- FindNeighbors(merged_sct, reduction = "harmony", dims = 1:40)
merged_sct <- FindClusters(merged_sct, resolution = 0.15)

DimPlot(merged_sct,  split.by = "stim", raster = F)
DimPlot(merged_sct,  raster = F)
